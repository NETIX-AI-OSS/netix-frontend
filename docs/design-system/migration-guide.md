...
netix-frontend 2.0.0 removed the importable UI surface and narrowed hooks to generic shared logic. Apps pinned to `#v1.0.2` keep working untouched; migrate when you bump.
...
| `netix-frontend/ui/<composite>` (data-table, form-modal, …) | `pnpm dlx shadcn add @netix/<name>` → `@/components/composites/<name>`                            |
| `netix-frontend/hooks` / `hooks/router`                     | `netix-frontend/hooks`; use the router-agnostic search-param binding with the app's router        |
...
Steps: bump the dep to `#v2.0.0`; collapse `globals.css` to `@import 'netix-frontend/styles.css';` and delete local token sheets; add the `@netix` entry to `components.json`; replace each UI import with its copied primitive/composite and keep shared hooks on `netix-frontend/hooks`; swap ThemeProvider to `netix-frontend/theme`; drop `tw-animate-css` (the lib stylesheet vendors the animation utilities). Tailwind v3 apps keep `netix-frontend/tokens/preset` and no longer need to scan the lib's dist in `content` globs — v2 ships no precompiled component CSS.
