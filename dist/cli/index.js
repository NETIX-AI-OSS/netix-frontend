...
var SKIP_EXACT = /* @__PURE__ */ new Set(["is_deleted", "organization_id", "url", "password", "attachment_urls"]);
var SKIP_SUFFIX = ["_details", "_metadata", "_urls"];
var LEAD_FIELDS = [
  "name",
  "title",
  "label",
...
function rowSchema(spec, endpoint) {
  const paths = spec.paths;
  const get = paths?.[endpoint]?.get;
  const responses = get?.responses;
  const content = responses?.["200"]?.content;
  const envelope = deref(spec, content?.["application/json"]?.schema);
  const envelopeProps = envelope?.properties;
  const results = envelopeProps?.results;
  if (!results) return void 0;
  return deref(spec, results.items);
}
function readSchemaColumns(specPath, endpoint) {
  if (!existsSync(specPath)) return [];
  let spec;
  try {
    spec = (0, import_yaml.parse)(readFileSync(specPath, "utf8"));
  } catch {
    return [];
  }
  ...
  return candidates.sort((a2, b) => rank(a2.field) - rank(b.field)).slice(0, MAX_COLUMNS);
}
var specPathFor = (root, slug) => join(root, "schema", `${slug}.yaml`);

// src/cli/transforms.ts
var pascalCase = (value) => value.split("-").map((part) => part[0]?.toUpperCase() + part.slice(1)).join("");
...
function rewriteDevPort(content, options) {
...
function requireService(options, key) {
...
var EMPTY_API = "    api: {},";
var API_BLOCK = /( {4}api: \{\n(?: {6}.*\n)*)( {4}\},)/;
function insertEnvApiEntry(content, key, service) {
...
${entry}    },`);
  if (API_BLOCK.test(content))
    return content.replace(API_BLOCK, (_, open, close) => `${open}${entry}${close}`);
  throw new Error("app/config/env.ts: no `api` map found to insert into");
}
var USER_UPSTREAM = "    '/user-api': `https://user.api.${baseDomain}`,\n";
function insertDevUpstream(content, key, service) {
...
var PAGES_ANCHOR = "// netix-pages:insert";
var ROUTES_ANCHOR = "              {/* netix-routes:insert */}";
var NAV_ANCHOR = "      // netix-nav:insert";
var insertAt = (content, file, anchor, entry) => {
  if (!content.includes(anchor)) throw new Error(`${file}: ${anchor.trim()} anchor not found`);
  return content.replace(anchor, `${entry}${anchor}`);
};
function insertLazyPageEntry(content, key) {
...
function insertRouteEntry(content, key) {
...
function insertNavEntry(content, key, service) {
...
var EMPTY_ORVAL = "export default defineConfig({})";
var ORVAL_CLOSING = /^\}\)$/m;
function insertOrvalBlock(content, key) {
  const { slug } = serviceNames(key);
  if (content.includes(`'${slug}':`)) return content;
  const block2 = buildOrvalBlock(key);
...
function rewriteRoutes(content, options) {
  for (const key of options.services) requireService(options, key);
  return options.services.reduce((acc, key) => insertRouteEntry(acc, key), content);
}
function rewriteNavigation(content, options) {
  return options.services.reduce(
...
// src/cli/service-files.ts
function buildServiceClient(key) {
...
function buildServiceClientTest(key) {
  const { slug } = serviceNames(key);
...
function buildServicePage(key, service, columns = []) {
  const { slug } = serviceNames(key);
  const { component } = pageNames(key);
  const title = service.title;
  const fromSchema = columns.length > 0;
  const columnSource = fromSchema ? `// Columns come from the API contract \u2014 schema/${slug}.yaml, the response schema of
...
function buildNavEntry(key, service) {
  const { path } = pageNames(key);
  return `      {
        label: '${service.title}',
...
function buildEnvApiEntry(key, service) {
  const { camel } = serviceNames(key);
  return `      ${camel}: serviceUrl('${service.apiSubdomain}', '/${key}-api'),
`;
}
function buildDevUpstreamEntry(key, service) {
...
var DEV_PORT_FILES = ["docker-compose.yaml", "Dockerfile"];
var DOC_FILES = ["README.md", "docs/AGENTS.md"];
function applyScaffoldTransforms(root, options) {
  ...
  for (const key of options.services) {
    ...
    if (service && key !== "user") {
      ...
      notes.push(
        columns.length ? `added ${key} page (${columns.length} columns from schema/${slug}.yaml)` : `added ${key} page (no spec yet \u2014 columns inferred from the first row)`
      );
    }
  }
...
function applyServiceWiring(root, services, manifest) {
  const notes = [];
  const writeIfMissing = (path, content, note2) => {
    const file = join(root, path);
    if (existsSync(file)) {
...
async function schemaPull({
  ...
  const result = { pulled: [], warnings: [], failures: [], generated: false };
  if (!keys.length) {
...
var DEFAULT_DEV_PORT = TEMPLATE_DEV_PORT;
var bail = (value) => {
  cancel("init cancelled");
  process.exit(1);
  throw value;
};
var isPort = (value) => /^\d+$/.test(value.trim()) && Number(value) >= 1 && Number(value) <= 65535;
...
async function init(flags, runner = run) {
  ...
  const portAnswer = flags.port ?? (flags.yes ? String(DEFAULT_DEV_PORT) : answer(
    await text({
      message: "Dev server port",
      placeholder: String(DEFAULT_DEV_PORT),
      defaultValue: String(DEFAULT_DEV_PORT),
      validate: (value) => !value || isPort(value) ? void 0 : "enter a port (1-65535)"
    })
  ));
  if (!isPort(portAnswer)) return fail(`dev server port must be 1-65535, got "${portAnswer}"`);
  const devPort = Number(portAnswer);
  const chosen = flags.services !== void 0 ? flags.services.split(",").map((s) => s.trim()).filter(Boolean) : flags.yes ? [] : answer(
    await multiselect({
      message: "Which services should this app talk to? (user management is always wired)",
      options: serviceKeys.filter((key) => key !== "user").map((key) => ({
        value: key,
  ...
  const skipped = [];
  const failed = [];
  const runStep = async (label, enabled, action) => {
    if (!enabled) {
      skipped.push(label);
      return;
    }
  ...
  const pending = (label) => skipped.includes(label) || failed.includes(label);
  const resume = [
    ...
    ...services.length > 0 && !pending("schema pull") && pending("pnpm generate:client") ? ["pnpm generate:client"] : []
  ];
  note(
    [
      `cd ${dir}`,
...
var bail2 = (value) => {
  cancel("service add cancelled");
  process.exit(1);
  throw value;
};
var answer2 = (value) => isCancel(value) ? bail2(value) : value;
async function serviceAdd(flags, runner = run) {
  const manifest = loadManifest();
  const serviceKeys = Object.keys(manifest.services);
  const cwd = flags.cwd ?? process.cwd();
  intro(import_picocolors2.default.inverse(" netix service add "));
    ...
    services = answer2(
      await multiselect({
        ...
        options: serviceKeys.map((key) => ({
          value: key,
          label: manifest.services[key]?.title ?? key,
          hint: wired.includes(key) ? "already wired \u2014 refreshes its schema" : manifest.services[key]?.backendRepo
        })),
        required: true
      })
    );
  ...
  const runStep = async (label, enabled, action) => {
    ...
    step.start(label);
    const ok = await action().catch(() => false);
    step.stop(ok ? label : `${label} ${import_picocolors2.default.red("failed")}`);
    if (!ok) failed.push(label);
  };
  await runStep("schema pull", flags.schemas !== false, async () => {
    const result = await schemaPull({ cwd, manifest, services, runner, log: log.info });
    for (const warning of result.warnings) log.warn(warning);
    for (const failure of result.failures) log.error(failure);
    return result.failures.length === 0;
  });
  try {
    for (const note2 of applyServiceWiring(cwd, services, manifest)) log.info(note2);
  } catch (error) {
    return fail(error instanceof Error ? error.message : String(error));
  }
  await runStep(
  ...
  note(
    [
      ...
      ...services.filter((key) => key !== "user").map(
        (key) => `${pageNames(key).file} \u2014 lists ${manifest.services[key]?.listEndpoint}; repoint LIST_ENDPOINT for another route`
      )
    ].join("\n"),
    "Next steps"
  );
  outro(`${import_picocolors2.default.green("\u2714")} ${services.join(", ")} wired`);
  return failed.length ? 1 : 0;
  function fail(message) {
    log.error(message);
    outro(import_picocolors2.default.red("service add failed"));
    return 1;
  }
}

// src/cli/index.ts
var HELP = `${import_picocolors3.default.bold("netix")} \u2014 scaffold and maintain NETIX frontend apps
...
`;
var splitServices = (args) => [
  ...new Set(
    args.flatMap((arg) => arg.split(",")).map((key) => key.trim()).filter(Boolean)
  )
];
...
`)
    ...
    for (const warning of result.warnings) process.stderr.write(`${import_picocolors3.default.yellow("warn")} ${warning}
`);
    for (const failure of result.failures) process.stderr.write(`${import_picocolors3.default.red("fail")} ${failure}
`);
