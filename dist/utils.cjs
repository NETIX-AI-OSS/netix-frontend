...
function commaSeparatedToArray(value, options) {
  const parts = value?.split(",").map((f) => f.trim()).filter((f) => f) || [];
  return options?.as === "string" ? parts : parts.map(Number);
}
function arrayToCommaSeparated(value) {
  return value?.join(",") || void 0;
}
function removeDuplicates(array) {
  return [...new Set(array)];
}
function filterIntersection(filter, restriction, apply = true, options) {
  if (!apply) return filter;
  const as = options?.as ?? "number";
  const filterArray = as === "string" ? commaSeparatedToArray(filter, { as }) : commaSeparatedToArray(filter);
  if (!filterArray.length) return arrayToCommaSeparated(restriction);
  if (!restriction) return filter;
  if (!restriction.length) return void 0;
  const intersection = restriction.filter(
    (asset) => filterArray.includes(as === "string" ? String(asset) : Number(asset))
  );
  return arrayToCommaSeparated(intersection);
}
function removeEmptyAttributes(obj) {
  return Object.entries(obj).filter(([, v]) => v !== void 0 && v !== null && v !== "").reduce((acc, [k, v]) => ({ ...acc, [k]: v }), {});
}
var DEFAULT_EMAIL_ERROR = "Please enter a valid email address";
function emailValidator(value, message = DEFAULT_EMAIL_ERROR) {
  if (!value) return { result: true };
  const result = /\S+@\S+\.\S+/.test(value);
  return { result, ...!result && { message } };
}
function getEnumOptions(obj, getLabel, options) {
  return Object.values(obj).filter((s) => !isNaN(Number(s))).map((s) => Number(s)).map((s) => ({
...
var getFullUserName = (user) => {
  ...
  if (!firstName && !lastName) return void 0;
  return `${firstName || ""} ${lastName || ""}`.trim();
};
var getUserNameInitials = (user) => {
...
exports.FULL_DATE_WITHOUT_WEEK_FORMAT = FULL_DATE_WITHOUT_WEEK_FORMAT;
exports.GMT_FORMAT = GMT_FORMAT;
...
exports.UPPERCASE_DATE_TIME_FORMAT = UPPERCASE_DATE_TIME_FORMAT;
exports.UTC_TIME_FORMAT = UTC_TIME_FORMAT;
exports.YEAR_OPTIONS = YEAR_OPTIONS;
exports.addDays = addDays;
exports.arrayToCommaSeparated = arrayToCommaSeparated;
exports.buildYearOptions = buildYearOptions;
exports.cn = cn;
...
exports.formatTimerClock = formatTimerClock;
exports.getEnumOptions = getEnumOptions;
exports.getFileMetadata = getFileMetadata;
exports.getFormattedShiftTime = getFormattedShiftTime;
exports.getFullDate = getFullDate;
exports.getFullDateByYear = getFullDateByYear;
exports.getFullDateTime = getFullDateTime;
exports.getFullDateWithoutTime = getFullDateWithoutTime;
exports.getFullGmtTime = getFullGmtTime;
...
exports.getStandardTimeWithSecond = getStandardTimeWithSecond;
exports.getTime = getTime;
exports.getTimeEstimate = getTimeEstimate;
exports.getUpperCaseDate = getUpperCaseDate;
exports.getUpperCaseDateTime = getUpperCaseDateTime;
exports.getUserNameInitials = getUserNameInitials;
exports.getWeekStartDate = getWeekStartDate;
exports.intervalToDuration = intervalToDuration;
exports.isScheduleDayVaild = isScheduleDayVaild;
exports.isScheduleDayValid = isScheduleDayValid;
exports.milliSecondsToDuration = milliSecondsToDuration;
exports.normalizeClockString = normalizeClockString;
