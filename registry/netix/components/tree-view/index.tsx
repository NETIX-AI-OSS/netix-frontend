...
const TreeView = <TData,>({
          ...
          >
            <div className="flex items-center gap-1">
              {hasChildren ? (
                <button
                  type="button"
                  className="h-7 w-7 rounded-item border border-transparent text-muted-foreground hover:bg-accent hover:text-accent-foreground"
              ...
              <button
                type="button"
                className={cn(
                  'min-h-8 flex-1 rounded-item px-2 py-1 text-start text-sm transition-colors',
  ...
  if (!items.length) {
    return (
      <div className={cn('rounded-panel border p-6 text-sm text-muted-foreground', className)}>
        {emptyText}
      </div>
    )
  }

  return <div className={cn('rounded-panel border p-2', className)}>{renderItems(items)}</div>
}
