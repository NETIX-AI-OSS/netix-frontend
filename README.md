...
| ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `netix-frontend/api`                                                                     | `createHttpClient`, `createMutator`, retry/error/envelope policy, TanStack Query and SWR retry policies, Sentry filter, `buildAuthConfig` + the canonical auth constants, `createDevLoginPrompt` (web dev), `createDevTokenManager` (react-native dev only) | web + RN  |
...
| `netix-frontend/hooks/permissions`                                                       | `usePermissionsFrom(user)` — the same permission state over a user you fetched yourself, with no TanStack Query import                                                                                                                                      | web + RN  |
