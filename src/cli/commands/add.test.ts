...
it('namespaces bare items into @netix and leaves qualified ones alone', () => {
  expect(
    qualifyItems(['confirm-modal', '@netix/data-table', 'https://x/r/y.json', 'file:../r/z.json']),
  ).toEqual(['@netix/confirm-modal', '@netix/data-table', 'https://x/r/y.json', 'file:../r/z.json'])
})

it('delegates to the pinned shadcn CLI and resolves with its exit code', async () => {
  const child = new EventEmitter()
  spawnMock.mockReturnValue(child)
  const pending = addItems(['confirm-modal'], ['--overwrite'])
  child.emit('close', 0)
